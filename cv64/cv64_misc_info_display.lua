...
function printVec3f(x, y, name, vec3f)
    gui.text(x, y, string.format("%s %5.1f, %5.1f, %5.1f", name, vec3f[0], vec3f[1], vec3f[2]))
end

-- print vec3s
function printVec3s(x, y, name, vec3s)
    gui.text(x, y, string.format("%s %04d, %04d, %04d", name, vec3s[0], vec3s[1], vec3s[2]))
end
...
function inGameplayLoop()
    ...
    if memory.read_u16_be(gameplayMgr) == 0005 and memory.readbyte(gameplayMgr + 0x9) == 03
    and memory.readbyte(gameplayMgr + 0xB) == 03 then
        return true
    else
        return false
    end
...
while true do
    if inGameplayLoop() == true then
        ...
        gui.text(0, vstart + vspace * 3, string.format("Speed: %f", getSpeedFromVelocityVec3f(player.velocity)))
        gui.text(0, vstart + vspace * 4, string.format("State: %s", player_states[player.state]))
        gui.text(0, vstart + vspace * 5, string.format("Sub-state: %d", player.sub_state))

        gui.text(0, vstart + vspace * 7, string.format("Enemy Target: %08X (%04X)", RDRAM_BASE_ADDR + player.enemy_target, player.enemy_target_ID))
        
        gui.text(0, vstart + vspace * 9, string.format("Week: %d, Day: %d", save.week, save.day))
        gui.text(0, vstart + vspace * 10, string.format("Hour: %d", save.hour))
        gui.text(0, vstart + vspace * 11, string.format("Minutes: %d", save.minutes))
        gui.text(0, vstart + vspace * 12, string.format("Seconds: %d", save.seconds))
        gui.text(0, vstart + vspace * 13, string.format("Milliseconds: %d", save.milliseconds))

        gui.text(0, vstart + vspace * 15, string.format("Object count: %d", getObjectCount()))
		gui.text(0, vstart + vspace * 16, string.format("Player push speed on wall contact: %f", getSpeedFromVelocityVec3f(player.wall_push_velocity)))
    end
    emu.frameadvance()
end
